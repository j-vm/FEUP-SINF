...
async function getData(token) {
  const data = Promise.all([getLogs(token), getProcesses(token)]);
        ...
        <Col sm="10">
          ...
          >
            ...
            {processes.map((process, i) => (
              <option key={i + 1} value={process.id}>
                {process.name}
              </option>
            ))}
          </Input>
