...
function AddStepForm({ onAdd }) {
    ...
    return (
        <Form>
            ...
            <Row className="mt-3">
                <Col>
                    <Label for="docType" className="font-weight-bold">Document Type</Label>
                    <Input type="select" name="docType" value={docType} onChange={(e) => {const { value } = e.target; setDocType(value);}} >
            ...
            <Row className="mt-3">
                <Col>
                    <Label for="type" className="font-weight-bold">Step Type</Label>
                    <FormGroup check>
                        <Label check>
                            <Input type="radio" name="type" value="wait" onChange={(e) => {const { value } = e.target; setType(value);}} />{' '}
                            Detect
                        </Label>
                    </FormGroup>
                    <FormGroup check>
                        <Label check>
                            <Input type="radio" name="type" value="emit" onChange={(e) => {const { value } = e.target; setType(value);}} />{' '}
                            Publish
                        </Label>
                    </FormGroup>
                </Col>
                <Col>
                    ...
                    <FormGroup check>
                        <Label check>
                            <Input type="radio" name="company" value="1" onChange={(e) => {const { value } = e.target; setCompany(value);}} />{' '}
                            Company 1
                        </Label>
                    </FormGroup>
                    <FormGroup check>
                        <Label check>
                            <Input type="radio" name="company" value="2" onChange={(e) => {const { value } = e.target; setCompany(value);}} />{' '}
                            Company 2
                        </Label>
                    </FormGroup>
                </Col>
...
export const Steps = (props) => {
    ...
    const onAdd = async (type, documentType, company, setDone) => {
        const result = await addStep(token, data.length+1, params.id, type, documentType, company);
